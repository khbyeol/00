...
public:
	ContriAccount(int _number, string _name, int _balance)
		:Account(_number, _name, (int)(_balance - _balance * 0.01)), 
		contribution((int)(_balance * 0.01))
	{
	}
	ContriAccount(int _number, string _name, int _balance, tm _newdate)
		:Account(_number, _name, _balance, _newdate), contribution(0)
	{
	}
